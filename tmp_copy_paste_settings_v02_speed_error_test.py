...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/gopro_settings_speed_test_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

# Статистика для каждой задержки
DELAYS = [0, 0.1, 1.0]  # Задержки в секундах
STATS = {delay: {
    'total_settings': 0,
    'applied_settings': 0,
    'failed_settings': 0,
...
async def apply_setting(camera_ip, setting_id, value, delay):
    """Применение настройки к камере"""
    STATS[delay]['total_settings'] += 1
    
    # Проверяем поддерживается ли значение
    if not is_value_supported(setting_id, value):
        STATS[delay]['skipped_settings'] += 1
        return False
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url) as response:
                ...
                if status == 200:
                    ...
                    STATS[delay]['applied_settings'] += 1
                    if delay > 0:
                        await asyncio.sleep(delay)  # Применяем задержку только если она больше 0
                    return True
                else:
                    logger.error(f"Failed to apply setting {setting_id}={value}, status: {status}")
                    STATS[delay]['failed_settings'] += 1
                    STATS[delay]['failed_list'].append((setting_id, value, status))
                    return False
                    
    except Exception as e:
        logger.error(f"Error applying setting {setting_id}={value}: {str(e)}")
        STATS[delay]['failed_settings'] += 1
        STATS[delay]['failed_list'].append((setting_id, value, 'error'))
        return False

async def verify_setting(camera_ip, setting_id, expected_value):
    """Проверка что настройка применилась корректно"""
...
def print_settings_report():
    ...
    for delay in DELAYS:
        ...
        if stats['failed_list']:
            logger.info(f"\nFailed settings list for {delay}s delay:")
        ...
        if total_attempted > 0:
            ...
            logger.info(f"\nSuccess rate for {delay}s delay: {success_rate:.2f}%")
...
async def main_async():
    ...
    try:
        ...
        # Применение настроек к остальным камерам с разными задержками
        for delay in DELAYS:
            ...
            for camera in cameras[1:]:
                ...
                for setting_id, value in current_settings.items():
                    if await apply_setting(camera['ip'], setting_id, value, delay):
